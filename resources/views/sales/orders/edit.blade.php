<x-app-layout>
    <div class="py-6">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Form Section -->
            <form method="POST" action="{{ route('sales.orders.update', $order->order_id) }}" id="orderForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="required_date" name="required_date" type="date" class="mt-1 block w-full" 
                                              :value="old('required_date', $order->required_date ? $order->required_date->format('Y-m-d') : '')" />
                                ...
                                <div class="item-row border border-gray-200 dark:border-gray-600 rounded-lg p-4 mb-4">
                                    <div class="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                                        <div class="md:col-span-2">
                                            ...
                                            <select name="items[{{ $itemIndex }}][product_id]" class="product-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" required>
                                                <option value="">Select Product</option>
                                                @foreach($products as $product)
                                                    <option value="{{ $product['id'] }}" data-price="{{ $product['price'] }}" data-stock="{{ $product['stock'] }}" 
                                                            {{ old("items.{$itemIndex}.product_id", $item->product_id) == $product['id'] ? 'selected' : '' }}>
                                        ...
                                        <div>
                                            ...
                                            <input type="number" name="items[{{ $itemIndex }}][quantity]" class="quantity-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" 
                                                   min="1" value="{{ old("items.{$itemIndex}.quantity", $item->quantity) }}" required>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Unit Price</label>
                                            <input type="number" name="items[{{ $itemIndex }}][unit_price]" step="0.01" class="unit-price-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" 
                                                   value="{{ old("items.{$itemIndex}.unit_price", $item->unit_price) }}" required>
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Discount</label>
                                            <input type="number" name="items[{{ $itemIndex }}][discount_amount]" step="0.01" class="discount-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" 
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <input type="text" name="items[{{ $itemIndex }}][notes]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" 
                                               value="{{ old("items.{$itemIndex}.notes", $item->notes) }}" placeholder="Item notes (optional)">
                                    </div>
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <x-text-input id="tax_amount" name="tax_amount" type="number" step="0.01" class="mt-1 block w-full" 
                                ...
                                <div>
                                    ...
                                    <x-text-input id="shipping_amount" name="shipping_amount" type="number" step="0.01" class="mt-1 block w-full" 
                                ...
                                <div>
                                    ...
                                    <x-text-input id="discount_amount" name="discount_amount" type="number" step="0.01" class="mt-1 block w-full" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <textarea id="shipping_address" name="shipping_address" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('shipping_address', $order->shipping_address) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea id="billing_address" name="billing_address" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('billing_address', $order->billing_address) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea id="notes" name="notes" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('notes', $order->notes) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea id="internal_notes" name="internal_notes" rows="3" 
                                          class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('internal_notes', $order->internal_notes) }}</textarea>
            ...
            function createItemRow(index) {
                return `
                    <div class="item-row border border-gray-200 dark:border-gray-600 rounded-lg p-4 mb-4">
                        ...
                        <div class="mt-3">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Notes</label>
                            <input type="text" name="items[${index}][notes]" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white" placeholder="Item notes (optional)">
                        </div>
                    </div>
                `;
            }
