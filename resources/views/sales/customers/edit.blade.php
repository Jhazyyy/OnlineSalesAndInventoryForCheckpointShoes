<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400">Update customer profile for
            ...
            <!-- Customer Form -->
            <form action="{{ route('sales.customers.update', $customer) }}" method="POST" enctype="multipart/form-data"
                class="mb-6">
                @csrf
                @method('PUT')
                ...
                {{-- <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label
                                    class="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('customer_type', $customer->customer_type) == 'individual' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                                    ...
                                    <div class="flex-1">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">Individual
                                            Customer</div>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">Personal customer account
                                        </div>
                                    </div>
                                    <svg class="w-5 h-5 text-blue-600 customer-type-check {{ old('customer_type', $customer->customer_type) == 'individual' ? '' : 'hidden' }}"
                            ...
                            <div>
                                <label
                                    class="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('customer_type', $customer->customer_type) == 'business' ? 'border-purple-500 bg-purple-50 dark:bg-purple-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                                    <input type="radio" name="customer_type" value="business" class="sr-only"
                                        ...
                                        onchange="toggleCustomerType()">
                                    <div class="flex-1">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">Business Customer
                                        </div>
                                        <div class="text-sm text-gray-500 dark:text-gray-400">Company or business
                                            account</div>
                                    </div>
                                    <svg class="w-5 h-5 text-purple-600 customer-type-check {{ old('customer_type', $customer->customer_type) == 'business' ? '' : 'hidden' }}"
                                        fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd"
                                            d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                            clip-rule="evenodd"></path>
                ...
                {{-- <div id="business-info"
                    class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg {{ old('customer_type', $customer->customer_type) == 'business' ? '' : 'hidden' }}">
                    <div class="p-6">
    ...
    <script>
        function toggleCustomerType() {
            ...
            if (businessRadio.checked) {
                ...
                document.querySelector('label:has(input[value="individual"])').className =
                    'flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 border-gray-300 dark:border-gray-600';
            } else {
                businessInfo.classList.add('hidden');
                companyName.removeAttribute('required');
                // Show individual check mark
                document.querySelector('label:has(input[value="individual"]) .customer-type-check').classList.remove(
                    'hidden');
                // Update styling
                document.querySelector('label:has(input[value="individual"])').className =
                    'flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 border-blue-500 bg-blue-50 dark:bg-blue-900/20';
                document.querySelector('label:has(input[value="business"])').className =
                    'flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 border-gray-300 dark:border-gray-600';
            }
        }
