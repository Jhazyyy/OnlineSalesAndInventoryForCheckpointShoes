    ...
    <div class="py-6">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400">View {{ ucfirst($taxDiscount->type) }} information</p>
                        </div>
            ...
            <div
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <!-- Calculation Details -->
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Calculation Details</h3>
                            <div class="space-y-3">
                                <div>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Method:</span>
                                    <p class="text-gray-900 dark:text-white">{{ ucfirst($taxDiscount->calculation_method) }}</p>
                                </div>
                                <div>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Rate/Amount:</span>
                                    <p class="text-gray-900 dark:text-white text-xl font-bold">{{ $taxDiscount->formatted_rate }}</p>
                                </div>
                                <div>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Priority:</span>
                                    <p class="text-gray-900 dark:text-white">{{ $taxDiscount->priority }}</p>
                                </div>
                                <div>
                                    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Compound:</span>
                                    <p class="text-gray-900 dark:text-white">{{ $taxDiscount->is_compound ? 'Yes' : 'No' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div>
                            ...
                            <p class="text-gray-900 dark:text-white">{{ ucfirst($taxDiscount->applies_to) }} Products</p>
                        </div>
                        ...
                        @if($taxDiscount->applies_to === 'specific')
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                                @if(count($categories) > 0)
                                    <div>
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Applicable Categories:</span>
                                ...
                                @if(count($products) > 0)
                                    <div>
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Applicable Products:</span>
                                        <ul class="mt-2 list-disc list-inside text-gray-900 dark:text-white">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="text-gray-900 dark:text-white">{{ $taxDiscount->created_at->format('M d, Y h:i A') }}</p>
                            </div>
                            <div>
                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Last Updated:</span>
                                <p class="text-gray-900 dark:text-white">{{ $taxDiscount->updated_at->format('M d, Y h:i A') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div
                ...
                <div class="p-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Profit Breakdown Calculator</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mb-4">Test how this {{ $taxDiscount->type }} affects profit calculations</p>
        ...
        function displayBreakdown(breakdown) {
            ...
            if (breakdown.taxes.length > 0) {
                ...
                breakdown.taxes.forEach(tax => {
                    html += `<div class="ml-4 text-gray-700 dark:text-gray-300">${tax.name} (${tax.rate}%): ₱${tax.amount.toFixed(2)}</div>`;
                });
                html += `<div class="text-blue-800 dark:text-blue-400"><strong>Total Tax:</strong> ₱${breakdown.total_tax.toFixed(2)}</div>`;
            }
            ...
            if (breakdown.discounts.length > 0) {
                ...
                breakdown.discounts.forEach(discount => {
                    html += `<div class="ml-4 text-gray-700 dark:text-gray-300">${discount.name} (${discount.rate}%): -₱${discount.amount.toFixed(2)}</div>`;
                });
                html += `<div class="text-green-800 dark:text-green-400"><strong>Total Discount:</strong> -₱${breakdown.total_discount.toFixed(2)}</div>`;
            }
