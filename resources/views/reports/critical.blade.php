                ...
                <div class="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        <button type="button" id="bulkReorderBtn" disabled
                                class="px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded text-sm transition disabled:opacity-50 disabled:cursor-not-allowed">
                    ...
                    <div class="flex justify-between items-center mb-4">
                        ...
                        @if(count($report['products'] ?? []) > 0)
                            <label class="flex items-center cursor-pointer">
                                <input type="checkbox" id="selectAll" class="rounded border-gray-300 text-orange-600 focus:ring-orange-500 mr-2">
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm sm:text-base">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    @foreach(['SKU / Product', 'Category', 'Current Qty', 'Critical Level', 'Suggested', 'Supplier', 'Reorder Action'] as $header)
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse(($report['products'] ?? []) as $product)
                                    <tr class="hover:bg-orange-50 dark:hover:bg-gray-700 transition" 
                                        ...
                                        data-suggested-qty="{{ $product->suggested_order_qty ?? max(1, (int)($product->shortage ?? 1)) }}">
                                        <td class="px-4 py-3">
                                            <input type="checkbox" 
                                                   class="product-checkbox rounded border-gray-300 text-orange-600 focus:ring-orange-500" 
                                                   data-product-id="{{ $product->product_id }}"
                                                   data-supplier-id="{{ $product->preferred_supplier_id ?? '' }}"
                                        ...
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $product->critical_level ?? '-' }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $product->suggested_order_qty ?? max(1, (int)($product->shortage ?? 1)) }}</td>
                                        <td class="px-4 py-3 sm:px-6">
                                        ...
                                        <td class="px-4 py-3 sm:px-6">
                                            <form method="POST" action="{{ route('reports.reorder.create') }}" class="flex flex-wrap gap-2 items-center">
                                                ...
                                                <input type="number" 
                                                       name="quantity" 
                                                       min="1" 
                                                       value="{{ max(1, (int)($product->suggested_order_qty ?? $product->shortage ?? 1)) }}"
            ...
            <script>
                ...
                function handleCheckboxChange(e) {
                    ...
                    if (checkbox.checked) {
                        ...
                        row.classList.add('bg-orange-100', 'dark:bg-orange-900');
                    } else {
                        selectedProducts.delete(productId);
                        
                        // Remove highlight from row
                        row.classList.remove('bg-orange-100', 'dark:bg-orange-900');
                    }
                ...
                document.getElementById('bulkReorderBtn')?.addEventListener('click', function() {
                    ...
                    if (!confirm(`Create a purchase order for ${productCount} critical item(s) from ${supplierName}?`)) {
                        return;
                    }
