<x-app-layout>
    <div class="py-6">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
                @php
                    $cards = [
                        ['label' => 'Refurbished', 'color' => 'from-blue-500 to-blue-600', 'value' => $report['summary']['total_refurbished_qty'] ?? 0, 'icon' => 'M4 4v6h6M20 20v-6h-6M4 10l6-6m4 16l6-6'],
                        ['label' => 'Damaged (Shipped)', 'color' => 'from-yellow-500 to-yellow-600', 'value' => $report['summary']['total_damaged_shipped_qty'] ?? 0, 'icon' => 'M9 12h6m2 5H7a2 2 0 01-2-2v-3m14 5a2 2 0 002-2v-3M5 10l-2-2m0 0l2-2m-2 2h6'],
                        ['label' => 'Damaged (Received)', 'color' => 'from-amber-500 to-amber-600', 'value' => $report['summary']['total_inbound_damaged_qty'] ?? 0, 'icon' => 'M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4'],
                        ['label' => 'Waste Qty', 'color' => 'from-red-500 to-red-600', 'value' => $report['summary']['total_waste_qty'] ?? 0, 'icon' => 'M19 7l-7 7-4-4'],
                        ['label' => 'Waste Value', 'color' => 'from-rose-500 to-rose-600', 'value' => '₱' . number_format($report['summary']['total_waste_value'] ?? 0, 2), 'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2']
                    ];
                @endphp
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-4 sm:p-6">
                    <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Blocked Items by Product</h3>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse(($report['products'] ?? []) as $row)
                                    <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition">
                                        <td class="px-4 py-3 sm:px-6 font-medium text-gray-900 dark:text-white">{{ $row['product_name'] }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $row['product_brand'] ?? 'N/A' }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $row['product_category'] ?? 'N/A' }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int) $row['refurbished_qty'] }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int) $row['damaged_shipped_qty'] }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int) $row['inbound_damaged_qty'] }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int) $row['waste_qty'] }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">₱{{ number_format($row['waste_value'] ?? 0, 2) }}</td>
                                    </tr>
                                @empty
                                    <tr>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-4 sm:p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">Purchase Order Master</h3>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">SKU</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Product Name</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Brand</th>
                                    <th class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Category</th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Ordered Qty</th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Total Cost</th>
                                    <th class="px-4 py-3 text-right text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Instock Qty</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 bg-white dark:bg-gray-800">
                                @forelse(($report['purchase_order_master'] ?? []) as $row)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        <td class="px-4 py-3 font-mono text-xs text-gray-900 dark:text-white">{{ $row->product_sku ?? 'N/A' }}</td>
                                        <td class="px-4 py-3 font-medium text-gray-900 dark:text-white">{{ $row->product_name }}</td>
                                        <td class="px-4 py-3 text-gray-800 dark:text-gray-300">{{ $row->product_brand }}</td>
                                        <td class="px-4 py-3 text-gray-800 dark:text-gray-300">{{ $row->product_category }}</td>
                                        <td class="px-4 py-3 text-right text-gray-800 dark:text-gray-300">{{ number_format($row->total_quantity ?? 0) }}</td>
                                        <td class="px-4 py-3 text-right text-gray-800 dark:text-gray-300">₱{{ number_format($row->total_cost ?? 0, 2) }}</td>
                                        <td class="px-4 py-3 text-right text-gray-800 dark:text-gray-300">{{ number_format($row->instock_qty ?? 0) }}</td>
                                    </tr>
                                @empty
                                    <tr>
    ...
    <div id="purchaseOrderModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-10 mx-auto p-5 border w-11/12 max-w-7xl shadow-lg rounded-md bg-white dark:bg-gray-800">
            <div class="flex justify-between items-center mb-4">
                ...
                <button onclick="closePurchaseOrderModal()" class="text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-100">
            ...
            <div class="mt-4 flex justify-end gap-3">
                <button onclick="closePurchaseOrderModal()" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg font-medium">
                    Close
                </button>
                <a id="downloadPurchaseOrderPdfLink" href="#" target="_blank" class="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg font-medium inline-block">
                    Download PDF
                </a>
            </div>
        </div>
    ...
    <script>
        function openPurchaseOrderModal() {
            ...
            const previewUrl = '{{ route('reports.preview-pdf', ['reportType' => 'blocked']) }}?start_date=' + startDate + '&end_date=' + endDate;
            const downloadUrl = '{{ route('reports.export-pdf', ['reportType' => 'blocked']) }}?start_date=' + startDate + '&end_date=' + endDate;
