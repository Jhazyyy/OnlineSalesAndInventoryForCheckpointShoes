    ...
    <div class="py-2">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
                ...
                @php
                    $cards = [
                        [
                            ...
                            'icon' => 'M3 12h18M9 18l-6-6 6-6',
                        ],
                        [
                            'label' => 'Out of Stock',
                            'color' => 'from-red-500 to-red-600',
                            'value' => $report['summary']['out_of_stock'] ?? 0,
            ...
            <div id="bulkActionBar"
                class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-4 hidden">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-4 sm:p-6">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-white">Products Needing Reorder
                        </h3>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm sm:text-base">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    <th
                                        class="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase w-12">
                                ...
                                @forelse(($report['products'] ?? []) as $product)
                                    <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition"
                                        ...
                                        <td class="px-4 py-3 sm:px-6">
                                            ...
                                            {{ $product->quantity <= 0 ? 'bg-red-100 text-red-800 dark:bg-red-200 dark:text-red-900' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-200 dark:text-yellow-900' }}">
                                                                     {{ $product->quantity }}
                                            </span> --}}
                                            ...
                                            bg-red-100 text-red-800 dark:bg-red-200 dark:text-red-900">{{ $product->quantity }}</span>
                                            @elseif ($product->quantity > 0 && $product->quantity <= 5)
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded text-xs 
                                            bg-red-100 text-red-500 dark:bg-yellow-200 dark:text-yellow-500">{{ $product->quantity }}</span>
                                            @elseif ($product->quantity > 5 && $product->quantity <= 10)
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded text-xs 
                                            bg-yellow-100 text-yellow-800 dark:bg-yellow-200 dark:text-yellow-900">{{ $product->quantity }}</span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">
                                            {{ $product->suggested_order_qty ?? 1 }}</td>
                                        <td class="px-4 py-3 sm:px-6">
                                        ...
                                        <td class="px-4 py-3 sm:px-6">
                                            <form method="POST" action="{{ route('reports.reorder.create') }}"
                                                ...
                                                <select name="supplier_id"
                                                    ...
                                                    @foreach ($suppliers as $s)
                                                        <option value="{{ $s->supplier_id }}"
                                                            {{ $product->preferred_supplier_id == $s->supplier_id ? 'selected' : '' }}>
                                                            {{ $s->supplier_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
