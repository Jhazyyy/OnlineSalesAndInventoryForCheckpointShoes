<x-app-layout>
    <div class="py-6">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
                ...
                <div class="p-4 sm:p-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow p-6">
                            <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Orders by Status</h3>
                            <div class="space-y-3">
                                @forelse($report['sales_by_status'] ?? [] as $status => $data)
                                    <div class="flex items-center justify-between p-3 bg-white dark:bg-gray-800 rounded">
                                        <span class="font-medium capitalize text-gray-900 dark:text-white">{{ $status }}</span>
                        ...
                        <!-- Top 10 Products -->
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow p-6">
                            <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Top 10 Best Selling Products</h3>
                            <div class="space-y-2">
                                ...
                                @empty
                                <p class="text-gray-500 dark:text-gray-400 text-center py-4">No products sold</p>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <!-- Top 10 Customers -->
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow p-6">
                            <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Top 10 Customers</h3>
                            <div class="space-y-2">
                                @forelse($report['top_customers'] ?? [] as $index => $customer)
                                    <div class="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-gray-800 rounded transition">
                                        <div class="flex items-center gap-3">
                                            <span class="font-bold text-lg text-gray-400">#{{ $index + 1 }}</span>
                                            <div>
                                                <p class="font-medium text-gray-900 dark:text-white">{{ $customer['customer_name'] ?? 'Unknown' }}</p>
                                ...
                                @empty
                                    <p class="text-gray-500 dark:text-gray-400 text-center py-4">No customer data</p>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <!-- Sales by Date -->
                        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg shadow p-6">
                            <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Daily Sales Summary</h3>
                            <div class="space-y-2 max-h-96 overflow-y-auto">
                                @forelse($report['sales_by_date'] ?? [] as $date => $data)
                                    <div class="flex items-center justify-between p-2 hover:bg-white dark:hover:bg-gray-800 rounded transition">
                                        <span class="text-sm text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($date)->format('M d, Y') }}</span>
                                        <div class="text-right">
                                ...
                                @empty
                                <p class="text-gray-500 dark:text-gray-400 text-center py-4">No sales data</p>
                                @endforelse
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-4 sm:p-6">
                    <h3 class="text-lg sm:text-xl font-bold mb-4 text-gray-900 dark:text-white">Sales Order Master</h3>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse(($report['product_sales'] ?? []) as $row)
                                    <tr class="hover:bg-blue-50 dark:hover:bg-gray-700 transition">
                                        <td class="px-4 py-3 sm:px-6 font-mono text-xs text-gray-900 dark:text-white">{{ $row->product_sku ?? 'N/A' }}</td>
                                        <td class="px-4 py-3 sm:px-6 font-medium text-gray-900 dark:text-white">{{ $row->product_name }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $row->product_brand }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ $row->product_category }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int)($row->total_quantity ?? 0) }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">₱{{ number_format($row->total_revenue ?? 0, 2) }}</td>
                                        <td class="px-4 py-3 sm:px-6 text-gray-800 dark:text-gray-300">{{ (int)($row->instock_qty ?? 0) }}</td>
                                    </tr>
                                @empty
                                    <tr>
