...
<head>
    ...
    <style>
        ...
        .muted { color: #666; }
        .summary { display: grid; grid-template-columns: repeat(5, 1fr); gap: 8px; margin: 12px 0 12px; }
        .card { border: 1px solid #ddd; padding: 8px; border-radius: 6px; }
        .breakdown { background: #f9f9f9; border: 1px solid #ddd; padding: 12px; margin: 12px 0 18px; border-radius: 6px; }
        .breakdown-title { font-weight: 700; margin-bottom: 8px; font-size: 13px; }
        .calc-row { display: flex; justify-content: space-between; padding: 3px 0; font-size: 11px; }
        .calc-divider { border-top: 1px solid #999; margin: 4px 0; padding-top: 4px; }
        .calc-highlight { font-weight: 700; color: #000; }
...
<body>
    ...
    <div class="breakdown">
        ...
        <div class="calc-row" style="margin-top: 8px; color: #6b46c1;">
            <span><strong>Net Revenue − COGS (₱{{ number_format($report['summary']['total_cost'] ?? 0, 2) }})</strong></span>
            <span><strong>= Gross Profit (₱{{ number_format($report['summary']['total_profit'] ?? 0, 2) }})</strong></span>
        </div>
