        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('notifications.list', ['filter' => 'all']) }}" 
                           class="px-4 py-2 text-sm font-medium rounded-lg {{ $filter === 'all' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition-colors">
                            All
                            <span class="ml-2 px-2 py-0.5 text-xs {{ $filter === 'all' ? 'bg-blue-600 dark:bg-blue-700' : 'bg-gray-500' }} text-white rounded-full">{{ $counts['all'] }}</span>
                        </a>
                        <a href="{{ route('notifications.list', ['filter' => 'unread']) }}" 
                           class="px-4 py-2 text-sm font-medium rounded-lg {{ $filter === 'unread' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition-colors">
                            Unread
                            <span class="ml-2 px-2 py-0.5 text-xs {{ $counts['unread'] > 0 ? 'bg-red-500' : 'bg-gray-500' }} text-white rounded-full">{{ $counts['unread'] }}</span>
                        </a>
                        <a href="{{ route('notifications.list', ['filter' => 'orders']) }}" 
                           class="px-4 py-2 text-sm font-medium rounded-lg {{ $filter === 'orders' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition-colors">
                            Orders
                            <span class="ml-2 px-2 py-0.5 text-xs bg-gray-500 text-white rounded-full">{{ $counts['orders'] }}</span>
                        </a>
                        <a href="{{ route('notifications.list', ['filter' => 'inventory']) }}" 
                           class="px-4 py-2 text-sm font-medium rounded-lg {{ $filter === 'inventory' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition-colors">
                            Inventory
                            <span class="ml-2 px-2 py-0.5 text-xs bg-gray-500 text-white rounded-full">{{ $counts['inventory'] }}</span>
                        </a>
                        <a href="{{ route('notifications.list', ['filter' => 'system']) }}" 
                           class="px-4 py-2 text-sm font-medium rounded-lg {{ $filter === 'system' ? 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700' }} transition-colors">
                            System
                            <span class="ml-2 px-2 py-0.5 text-xs bg-gray-500 text-white rounded-full">{{ $counts['system'] }}</span>
                        </a>
                    </div>

                    <!-- Actions -->
                    <div class="flex gap-2">
                        <button onclick="markAllAsRead()" class="px-4 py-2 text-sm font-medium rounded-lg text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
                            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                            </svg>
                            Mark all as read
                        </button>
                        <button onclick="deleteAllNotifications()" class="px-4 py-2 text-sm font-medium rounded-lg text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 transition-colors">
                            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if($notifications->isEmpty())
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
                    <svg class="w-16 h-56 mx-auto text-gray-400 dark:text-gray-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                <div class="space-y-3">
                    @foreach($notifications as $notification)
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border-l-4 {{ $borderClass }} hover:shadow-md transition-shadow notification-item" data-id="{{ $notification->id }}">
                            <div class="p-4">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="mt-3 flex gap-2">
                                            @if($notification->link)
                                                <a href="{{ $notification->link }}" class="px-3 py-1.5 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-md transition-colors">
                                                    View Details
                                                </a>
                                            @endif
                                            @if($notification->isUnread())
                                                <button onclick="markAsRead({{ $notification->id }})" class="px-3 py-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md transition-colors">
                                                    Mark as Read
                                                </button>
                                            @endif
                                            <button onclick="deleteNotification({{ $notification->id }})" class="px-3 py-1.5 text-xs font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors">
                                                Delete
                                            </button>
                                        </div>
                                    </div>
