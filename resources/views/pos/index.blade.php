                ...
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p class="text-gray-600 dark:text-gray-400">View all sales transactions</p>
                        </div>
                                ...
                                <div class="rounded-full bg-blue-100 dark:bg-blue-900 p-3">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="4"
                                            d="M7 4h6a5 5 0 0 1 0 10H7m0-10v16m0-8h6m-6-4h6" />
                                    </svg>
                                </div>
                            </div>
                            <div class="ml-4">
                                ...
                                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                                    ₱{{ number_format($todaySummary['total_sales'], 2) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" name="search" value="{{ request('search') }}" 
                                    placeholder="Order #, customer, payment..."
                            ...
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th
                                        ...
                                        Payment</th>
                                    <th
                                        class="px-6 py-3 text-center text-xs font-mono text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Payment Status</th>
                                    <th
                                        class="px-6 py-3 text-center text-xs font-mono text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                        Subtotal</th>
                                ...
                                @forelse($orders as $order)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <div class="text-xs text-gray-500 dark:text-gray-400">
                                                {{ $order->customer->phone ?? 'N/A' }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900 dark:text-white">
                                                {{ $order->items->count() }} item(s)</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap">
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $order->payment_status === 'paid' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' }}">
                                                {{ ucfirst($order->payment_status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <div class="text-sm text-gray-700 dark:text-gray-300">
                                                ₱{{ number_format($order->subtotal ?? $order->total_amount - ($order->tax_amount ?? 0) + ($order->discount_amount ?? 0), 2) }}
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="10"
                                            ...
                                            <p class="mt-2 text-sm">No sales, name, payment, or other records found.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
