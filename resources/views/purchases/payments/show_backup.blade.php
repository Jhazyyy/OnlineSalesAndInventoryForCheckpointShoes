            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                        ...
                        <div>
                            ...
                            <dl class="space-y-2">
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->payment_number }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Payment Date</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->payment_date ? $purchasePayment->payment_date->format('M d, Y') : 'N/A' }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Amount</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">₱{{ number_format($purchasePayment->amount, 2) }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Unused Amount</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">₱{{ number_format($purchasePayment->unused_amount, 2) }}</dd>
                                </div>
                                @if($purchasePayment->bank_charges)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Bank Charges</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">₱{{ number_format($purchasePayment->bank_charges, 2) }}</dd>
                                </div>
                                @endif
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Status</dt>
                                    <dd><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $purchasePayment->status_badge_class }}">
                        ...
                        <div>
                            ...
                            <dl class="space-y-2">
                                <div>
                                    ...
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->supplier->supplier_name }}</dd>
                                </div>
                                @if($purchasePayment->payment_mode)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Payment Mode</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->payment_mode_display }}</dd>
                                </div>
                                @endif
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Payment Method</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->payment_method_display }}</dd>
                                </div>
                                @if($purchasePayment->reference_number)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Reference Number</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->reference_number }}</dd>
                                </div>
                                @endif
                                @if($purchasePayment->bill_number)
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Bill Number</dt>
                                    <dd class="text-sm text-gray-900 dark:text-white">{{ $purchasePayment->bill_number }}</dd>
                                </div>
                                @endif
                            </dl>
                        </div>
                    ...
                    <div class="mt-6">
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-300">{{ $purchasePayment->notes }}</p>
                    </div>
                    @endif

                    <!-- Related Bills -->
                    @if(isset($relatedBills) && $relatedBills->count() > 0)
                    <div class="mt-8">
