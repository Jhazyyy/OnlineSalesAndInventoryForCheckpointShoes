<x-app-layout>
    <div class="py-6">
        <div class="w-full mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Purchase Receive: {{ $receive->receive_number }}</h2>
                            <p class="text-gray-600 dark:text-gray-400">Update purchase receive details</p>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-3 mt-4 sm:mt-0">
                            <a href="{{ route('inventory.purchase-receives.show', $receive->receive_id) }}" 
            ...
            <!-- Main Form -->
            <form action="{{ route('inventory.purchase-receives.update', $receive->receive_id) }}" method="POST" id="receiveForm">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                    ...
                                    <div>
                                        ...
                                        <select id="supplier_id" name="supplier_id" required
                                            ...
                                            @foreach($suppliers as $supplier)
                                                <option value="{{ $supplier['id'] }}" {{ old('supplier_id', $receive->supplier_id) == $supplier['id'] ? 'selected' : '' }}>
                                                    {{ $supplier['name'] }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div>
                                        ...
                                        <select id="purchase_order_id" name="purchase_order_id" required
                                            ...
                                            @foreach($purchase_orders as $order)
                                                <option value="{{ $order['id'] }}" data-supplier-id="{{ $order['supplier_id'] ?? '' }}" {{ old('purchase_order_id', $receive->purchase_order_id) == $order['id'] ? 'selected' : '' }}>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Purchase Receiver#</label>
                                    ...
                                    <!-- Received Date -->
                                    <div>
                                        <label for="receive_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                                            Received Date <span class="text-red-500">*</span>
                                        </label>
                                        <input type="date" id="receive_date" name="receive_date" 
                                               value="{{ old('receive_date', $receive->receive_date->format('Y-m-d')) }}" required
                                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                                        @error('receive_date')
                                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                        @enderror
                                        ...
                                        <div class="flex">
                                            ...
                                            <div class="ml-3">
                                                <p class="text-sm text-orange-700">
                                                    You can reload items from the purchase order or manually adjust quantities.
                                                    <button type="button" onclick="loadPurchaseOrderItems()" class="text-blue-600 hover:text-blue-800 underline ml-1">
                                                        Reload PO items
                                                    </button>
                                                </p>
                                            </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">Items & Description</h3>
                                    <button type="button" onclick="addItemRow()" class="inline-flex items-center px-3 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700" id="itemsTable">
                                        <thead class="bg-gray-50 dark:bg-gray-700">
                                            <tr>
                                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Items & Description</th>
                                                ...
                                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">In Transit</th>
                                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Quantity to Receive</th>
                                ...
                                <div id="noItemsMessage" class="text-center py-8 text-gray-500 dark:text-gray-400" style="display: none;">
                        ...
                        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                            <div class="p-6">
                                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Notes (For Internal Use)</h3>
                                
                                <div class="space-y-4">
                                    <div>
                                        <label for="receiving_notes" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Receiving Notes</label>
                                    ...
                                    <div>
                                        <label for="receiver_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Receiver Name</label>
                                ...
                                <div class="space-y-3">
                                    ...
                                    @else
                                        <div class="text-center py-4">
                                            <p class="text-sm text-gray-500 dark:text-gray-400">This receive cannot be edited due to its current status.</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="p-6">
                                ...
                                <div class="space-y-3">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="text-sm font-medium text-gray-900 dark:text-white" id="totalItems">{{ $receive->items->count() }}</span>
                                    </div>
                                    
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">Total Quantity:</span>
                                        <span class="text-sm font-medium text-gray-900 dark:text-white" id="totalQuantity">{{ number_format($receive->total_quantity_received) }}</span>
                                    </div>
                                    
                                    <div class="flex justify-between">
                                        <span class="text-sm text-gray-500 dark:text-gray-400">Total Value:</span>
                                        <span class="text-sm font-medium text-gray-900 dark:text-white" id="totalValue">₱{{ number_format($receive->total_amount_received, 2) }}</span>
                                    </div>
