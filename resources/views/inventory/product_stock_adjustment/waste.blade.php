                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                <h4 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">Important Notice
                                </h4>
                                <p class="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                                    This will permanently reduce the inventory quantities. Please ensure all details are
                                    accurate before proceeding.
                                    Common reasons include damaged goods, theft, or quality control issues.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($errors->any())
                        <div
                            class="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded relative dark:bg-red-900 dark:border-red-700 dark:text-red-100">
                    ...
                    <form method="POST" action="{{ route('inventory.product_stock_adjustment.waste.process') }}">
                        @csrf
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700 p-6 rounded-lg mb-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <select id="product_id" name="product_id"
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->product_id }}"
                                                ...
                                                ({{ number_format($product->quantity) }} available)
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div id="current-stock-display" class="hidden">
                                    ...
                                    <div
                                        class="mt-1 p-3 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-800 rounded-md">
                        ...
                        <div
                            class="bg-red-50 dark:bg-red-900/20 p-6 rounded-lg border border-red-200 dark:border-red-800 mb-6">
                            <h4 class="text-md font-semibold mb-4 text-red-800 dark:text-red-200">Waste/Damage Details
                            </h4>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Enter the number of units
                                        to remove from inventory</p>
                                </div>

                                <!-- Unit Cost -->
                                <div>
                                    <x-input-label for="unit_cost" :value="__('Unit Cost (Optional)')" />
                                    <x-text-input id="unit_cost" name="unit_cost" type="number" step="0.01"
                                        min="0" class="mt-1 block w-full" :value="old('unit_cost')" placeholder="0.00" />
                                    <x-input-error class="mt-2" :messages="$errors->get('unit_cost')" />
                                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Cost per unit for loss
                                        calculation</p>
                                </div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <select id="reason" name="reason"
                                        ...
                                        <option value="Damaged during handling"
                                            {{ old('reason') == 'Damaged during handling' ? 'selected' : '' }}>Damaged
                                            during handling</option>
                                        <option value="Quality control failure"
                                            {{ old('reason') == 'Quality control failure' ? 'selected' : '' }}>Quality
                                            control failure</option>
                                        <option value="Water damage"
                                            {{ old('reason') == 'Water damage' ? 'selected' : '' }}>Water damage
                                        </option>
                                        <option value="Fire damage"
                                            {{ old('reason') == 'Fire damage' ? 'selected' : '' }}>Fire damage</option>
                                        <option value="Theft" {{ old('reason') == 'Theft' ? 'selected' : '' }}>Theft
                                        </option>
                                        <option value="Manufacturing defect"
                                            {{ old('reason') == 'Manufacturing defect' ? 'selected' : '' }}>
                                            Manufacturing defect</option>
                                        <option value="Customer return - damaged"
                                            {{ old('reason') == 'Customer return - damaged' ? 'selected' : '' }}>
                                            Customer return - damaged</option>
                                        <option value="Obsolete inventory"
                                            {{ old('reason') == 'Obsolete inventory' ? 'selected' : '' }}>Obsolete
                                            inventory</option>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="movement_date" name="movement_date" type="datetime-local"
                                        class="mt-1 block w-full" :value="old('movement_date', now()->format('Y-m-d\TH:i'))" />
                        ...
                        <div class="mb-6">
                            ...
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Detailed description will help
                                with future prevention and insurance claims</p>
                        </div>

                        <!-- Impact Summary -->
                        <div id="impact-summary"
                            class="bg-orange-50 dark:bg-orange-900/20 p-6 rounded-lg border border-orange-200 dark:border-orange-800 mb-6 hidden">
                            <h4 class="text-md font-semibold mb-4 text-orange-800 dark:text-orange-200">Impact Summary
                            </h4>

                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <span
                                        class="text-sm font-medium text-orange-700 dark:text-orange-300">Product:</span>
                                    <div id="impact-product" class="text-orange-900 dark:text-orange-100">-</div>
                                </div>
                                <div>
                                    <span class="text-sm font-medium text-orange-700 dark:text-orange-300">Quantity
                                        Lost:</span>
                                    <div id="impact-quantity" class="text-orange-900 dark:text-orange-100">-</div>
                                </div>
                                <div>
                                    <span class="text-sm font-medium text-orange-700 dark:text-orange-300">Financial
                                        Impact:</span>
                            ...
                            <div class="mt-4">
                                <span class="text-sm font-medium text-orange-700 dark:text-orange-300">New Stock
                                    Level:</span>
                                <div class="mt-1">
                                    <span id="impact-before" class="text-orange-900 dark:text-orange-100">-</span>
                                    <svg class="inline w-4 h-4 mx-2" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M17 8l4 4m0 0l-4 4m4-4H3"></path>
                                    </svg>
                                    <span id="impact-after"
                                        class="text-orange-900 dark:text-orange-100 font-semibold">-</span>
                                </div>
                            </div>
                        </div>
